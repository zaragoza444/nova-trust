...
function requireEnv(name: string) {
  ...
  if (!value) {
    throw new Error(`${name} is required for Z Block Chain liquidity setup`);
  }
  return value;
}
...
async function loadConfig(walletAddress: string): Promise<BootstrapConfig> {
  ...
  return {
    ...
    networkName: process.env.ZBC_NETWORK_NAME ?? "Z Block Chain",
    outputPath:
...
async function main() {
  ...
  console.log("Z Block Chain production liquidity bootstrap complete");
  console.log(JSON.stringify(manifest, null, 2));
}

main().catch((error) => {
  console.error("Z Block Chain production liquidity bootstrap failed", error);
  process.exit(1);
});
