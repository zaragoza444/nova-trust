#!/usr/bin/env python3
"""Remote 1M BTC clone mint on the Nova Trust VPS over SSH."""
...
def main() -> None:
    branch = os.environ.get("VPS_DEPLOY_BRANCH", "cursor/mint-clone-btc-4f28")
...
bash scripts/mint-clone-btc-1m-vps.sh
"""

    client = connect()
    try:
        print("Connected to VPS. Syncing repo and minting 1M BTC clone...")
        ...
        if code == 0 and manifest_body.strip():
            ...
            btc = next(
                (token for token in parsed.get("cloneTokens", []) if token.get("symbol") == "BTC"),
                None,
            )
            print(
                "Saved production manifest locally "
                f"(BTC clone address={btc.get('address') if btc else 'unknown'})."
            )
        else:
            print("BTC mint completed but manifest fetch failed:", err.strip())
    finally:
        client.close()
