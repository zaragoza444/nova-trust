...
export function loadTradableTokenRegistry(): TradableTokenRegistry {
  const registryPath =
    process.env.Z_TOKEN_REGISTRY_PATH ?? "config/tokens/z-block-chain-tradable-tokens.v1.json";
  return readJson<TradableTokenRegistry>(registryPath);
}

export function loadTradingPlatformRegistry(): TradingPlatformRegistry {
  return readJson<TradingPlatformRegistry>("config/compliance/z-trading-platforms.v1.json");
}
