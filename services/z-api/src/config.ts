...
export function loadZApiConfig(): ZApiConfig {
  const corsOrigin = process.env.Z_CORS_ORIGIN ?? process.env.NOVA_CORS_ORIGIN ?? "http://localhost:3100";
  ...
  return {
    ...
    host: process.env.Z_API_HOST ?? process.env.NOVA_API_HOST ?? "127.0.0.1",
